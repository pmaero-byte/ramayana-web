    ...
    public static class Day39RuntimeProbe
    {
        ...
        public static void Run()
        {
            ...
            using (var sw = new StreamWriter(ReportPath, false))
            {
                ...
                // 1. KandaTree + progression probes.
                sw.WriteLine("[KandaTree]");
                sw.WriteLine("  Count=" + KandaTree.KandaCount);
                sw.WriteLine("  Entries: " + string.Join(" | ", KandaTree.Entries.Select(e => $"{e.Order}:{e.KandaId}").ToArray()));
                var corpusProbe = KandaTree.Entries.Select(e => e.KandaId + ":" + KandaTree.GetCorpusFile(e.KandaId));
                sw.WriteLine("  CorpusFiles: " + string.Join(", ", corpusProbe.ToArray()));
                                sw.WriteLine();
                ...
                // Use a real slot key so GetMostRecentSave() can see the test save.
                // Preserve and restore any existing save to avoid collateral damage.
                string testSlot = Jambudweep.Ramayana.Data.SaveKeys.Slot1;
                SaveData prevSlot1 = SaveSystem.Load(testSlot);
                SaveData testSave = new SaveData { currentActId = "yuddhakanda-war", visitedScenes = new string[0] };
                SaveSystem.Save(testSlot, testSave);
                sw.WriteLine("  LockTestYuddha(emptyVisited)=" + KandaPermissions.IsUnlocked("yuddha-kanda"));
                // Clear currentActId now -> dev-mode unlock all kandas.
                testSave.currentActId = string.Empty;
                SaveSystem.Save(testSlot, testSave);
                sw.WriteLine("  DevModeYuddha(noActId)=" + KandaPermissions.IsUnlocked("yuddha-kanda"));
                // Restore previous slot-1 state if it existed, else delete.
                if (prevSlot1 != null)
                {
                    SaveSystem.Save(testSlot, prevSlot1);
                }
                else
                {
                    SaveSystem.DeleteSlot(testSlot);
                }
                sw.WriteLine();
