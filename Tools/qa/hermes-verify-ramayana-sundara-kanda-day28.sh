...
FILE="$ROOT/Assets/Resources/Ramayana/moments_sundara_kanda.json"
PASS=0; FAIL=0
p(){ echo "  PASS: $*"; PASS=$((PASS+1)); }
f(){ echo "  FAIL: $*"; FAIL=$((FAIL+1)); }

[ -f "$FILE" ] && p "moments_sundara_kanda.json exists" || f "moments_sundara_kanda.json missing"
...
echo "$SRC" | grep -q '"kanda": "sundara-kanda"' && p "kanda=sundara-kanda" || f "missing kanda"
echo "$SRC" | grep -q 'vc_sundara_' && p "voiceCueId pattern" || f "voiceCueId missing"
...
for char in hanuman rama sita ravana trijata mandodari surasa sinhika; do
  echo "$SRC" | python3 -c "
import json,sys
...
echo ""
echo "  $PASS passed, $FAIL failed  (Day 28)"
