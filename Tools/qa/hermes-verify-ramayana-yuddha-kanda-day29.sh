...
FILE="$ROOT/Assets/Resources/Ramayana/moments_yuddha_kanda.json"
PASS=0; FAIL=0
p(){ echo "  PASS: $*"; PASS=$((PASS+1)); }
f(){ echo "  FAIL: $*"; FAIL=$((FAIL+1)); }

[ -f "$FILE" ] && p "moments_yuddha_kanda.json exists" || f "moments_yuddha_kanda.json missing"
...
echo "$SRC" | grep -q '"kanda": "yuddha-kanda"' && p "kanda=yuddha-kanda" || f "missing kanda"
echo "$SRC" | grep -q 'vc_yuddha_' && p "voiceCueId pattern" || f "voiceCueId missing"
...
for char in rama sita hanuman ravana vishwamitra vibhishana kumbhakarna indrajit bharata; do
  echo "$SRC" | python3 -c "
import json,sys
...
bash -n "$ROOT/Tools/qa/hermes-verify-ramayana-sundara-kanda-day28.sh" 2>/dev/null && p "Day 28 verifier bash -n" || f "Day 28 verifier bash -n"
bash -n "$ROOT/Tools/qa/hermes-verify-ramayana-kishkindha-kanda-day27.sh" 2>/dev/null && p "Day 27 verifier bash -n" || f "Day 27 verifier bash -n"
echo ""
echo "  $PASS passed, $FAIL failed  (Day 29)"
