...
FILE="$ROOT/Assets/Resources/Ramayana/moments_uttara_kanda.json"
PASS=0; FAIL=0
p(){ echo "  PASS: $*"; PASS=$((PASS+1)); }
f(){ echo "  FAIL: $*"; FAIL=$((FAIL+1)); }

[ -f "$FILE" ] && p "moments_uttara_kanda.json exists" || f "moments_uttara_kanda.json missing"
...
echo "$SRC" | grep -q '"kanda": "uttara-kanda"' && p "kanda=uttara-kanda" || f "missing kanda"
echo "$SRC" | grep -q 'vc_uttara_' && p "voiceCueId pattern" || f "voiceCueId missing"
...
for char in rama sita lakshmana bharata kaikeyi mandodari shabari trijata lava kusha; do
  echo "$SRC" | python3 -c "
import json,sys
...
echo ""
echo "  $PASS passed, $FAIL failed  (Day 30)"
